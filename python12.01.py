...
def detectCorner(img):
    ...
    linesP = cv2.HoughLinesP(edges, 1, numpy.pi / 180,
...
def checkLineExisted(img):
    if detectYellow(img) == 109:
        return 200
    else:
        return 201


stair_low = [green_low, red_low, blue_low, red_low, green_low, white_low]
stair_up = [green_up, red_up, blue_up, red_up, green_up, white_up]


def getAvgDegree(contour):
    vx, vy, x, y = cv2.fitLine(contour, cv2.DIST_L2, 0, 0.01, 0.01)
    rad = math.atan(vy/vx)
    # x2, y2 = x+100*math.cos(rad), y+100*math.sin(rad)
    # x, y, x2, y2 = map(int, [x, y, x2, y2])
    return rad*(180/math.pi)


def checkStair(img, rx):
    color = rx-90
    img_hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    mask = cv2.inRange(img_hsv, tuple(
        stair_low[color]), tuple(stair_up[color]))
    img_mask = cv2.bitwise_and(img, img, mask=mask)
    contours, _ = cv2.findContours(
        mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)

    img3 = np.zeros((viewSize[1], viewSize[0]), np.uint8)
    ...
    cv2.drawContours(img3, contour, -1, (255, 255, 255))

    cv2.imshow("sss", img3)
    lines = cv2.HoughLinesP(img3, 1, np.pi / 180, 1,
    ...
    for line in lines:
        ...
        if (x1 == x2 and (x1 == 0 or x1 == viewSize[0])) or (y1 == y2 and (y1 == 0 or y1 == viewSize[1])):
            continue
        linesWithAngle.append((x1, y1, x2, y2, angle))
        cv2.line(img3, (x1, y1), (x2, y2), (255, 0, 255))
    lineM = max(linesWithAngle, key=lambda l: abs(l[0]-l[2]))
    x1, y1, x2, y2, angle = lineM
    img3 = np.zeros((viewSize[1], viewSize[0]), np.uint8)
    cv2.line(img3, (x1, y1), (x2, y2), (255, 0, 255))
    cv2.imshow("final", img3)

    lineDeg = angle*(180/math.pi)
    #lineDeg = round(getSubDegree(90, lineDeg), 1)
    ...
    if lineDeg > a:
        ...
        rect = getColorObject(img, stair_low[color], stair_up[color])
        chkY = rect['y']+rect['h'] 
        print(chkY)
        #chkY *= 3
        cv2.rectangle(img, (rect['x'], rect['y']), (rect['x'] +
                  rect['w'], rect['y']+rect['h']), (255, 255, 0), 2)
        cv2.imshow("rect",img)
        if (chkY < viewSize[1]*0.75 and rx<93) or (rx>=93 and chkY<viewSize[1]*0.8):
            return 94
        else:
            return 93
...
actionFunc = {150: detectLine, 151: detectDirection,
              152: detectRoomName, 153: gotoObject, 154: pickObject, 155: putObject, 156: gotoEdge, 157: detectDanger, 158: checkLineExisted,159: traceLine_EXIT}
